const dataArray = [
    {"question": "HOW ARE YOU, <br> REALLY?"},
    {"question": "WHAT ARE YOUR NEW FAVORITE MOVIES? SHARE ALL OF THEM."},
    {"question": "WHAT IS THE MOST MEMORABLE MOMENT IN YOUR 26TH YEAR THAT MADE YOU THE HAPPIEST?"},
    {"question": "WHAT CHALLENGES DID YOU ENCOUNTER, AND HOW DO YOU THINK YOU HANDLED THEM?"},
    {"question": "HOW DO YOU THINK YOU'VE GROWN OR CHANGED IN THE LAST YEAR?"},
    {"question": "WHAT ARE YOU MOST GRATEFUL FOR?"},
    {"question": "WRITE A LETTER TO 27-YEAR-OLD GABBY."},
    {"question": "BONUS QUESTION: <br><br>WILL YOU STILL LOVE ME IN THE MORNING? (char)"},
    {"question": "TIME TO GET FEELY."},
];

const letterContent = `
    <h1 id="top"> DEAR GIANNE, </h1><br>
    <p> Happy Birthday!</p> <br>
    <p> 
    I am running out of ideas, so here's a special edition of WNRS for you. JK! Those questions are the ones I wanted to ask but wasn't able to because..life. Seriously, time sucks. I wish we could go back to 2008 when time was unlimited, and our schedules revolved around chikas, daydreaming, and just laughing out of nowhere.
    </p><br>
    <p>
    But life happens, and it happens fast. Still, I'm grateful for all our quick messages, IG reels, and TikToks. Seems to be the littlest things, but they really make my day. You always make my day.
    </p><br>
    <p>
    I'm proud of you, big girl ka na! Watching you from afar gives me happiness and excitement. I know you have your own battles, but you're killing it, and you're killing it gracefully.
    </p><br>
    <p>
    I'm lucky to have known the little Gianne, the workaholic G, and now the goal-getter Gabby. Whoever is next, I'm gonna be rooting for her too.
    </p><br>
    <p>
    Happy birthday. Here's to growing up but never growing apart    (100)
    </p><br>
    <p>PS. I miss you forreal</p><br><br>
    <h1 id="bottom"> LOVE, SAMENG </h1><br>
    
    </div>
`
window.letterContent = letterContent;
window.dataArray = dataArray;
